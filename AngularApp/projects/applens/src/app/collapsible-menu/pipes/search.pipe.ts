...
export class SearchPipe implements PipeTransform {
    transform(items: CollapsibleMenuItem[], searchString: string) {
        return searchString && items ? items.filter(item => item.label.toLowerCase().indexOf(searchString.toLowerCase()) >= 0 || item.id.toLowerCase().indexOf(searchString.toLowerCase()) >= 0 || item.metadata && item.metadata.toLowerCase().indexOf(searchString.toLowerCase()) >= 0) : items;
    }
}
